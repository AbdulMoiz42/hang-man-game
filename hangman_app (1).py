...
# ----------------------------------------------------------------------------
    ...
    # Secret word display — correctly guessed letters are highlighted in green
    letter_spans = []
    for ch in st.session_state.word:
        if ch in st.session_state.guessed_letters:
            letter_spans.append(f'<span style="color:#2f9e44;">{ch}</span>')
        else:
            letter_spans.append('<span style="color:#adb5bd;">_</span>')
    display_word = " ".join(letter_spans)
    st.markdown(f'<div class="secret-word">{display_word}</div>', unsafe_allow_html=True)

    # Clear feedback about the most recent guess
    if st.session_state.last_guess is not None:
        if st.session_state.last_guess_correct:
            st.success(f"✅ Correct! '{st.session_state.last_guess.upper()}' is in the word — check above where it filled in.")
        else:
            st.error(f"❌ Wrong! '{st.session_state.last_guess.upper()}' is not in the word.")
